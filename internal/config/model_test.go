...
func TestResolveBuiltinDefault(t *testing.T) {
	...
	mc := m.Resolve(domain.RepoRedeAgenda, 0)
...
func TestResolveFileDefault(t *testing.T) {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).Model; got != "claude-sonnet-4-6" {
...
func TestResolvePrecedence(t *testing.T) {
	...
	cfg := &Config{
		Model: ModelSection{
			...
			Repos: map[string]ModelOverride{
				domain.RepoRedeAgenda: {Model: "repo-web-model"},
			},
			Issues: map[string]ModelOverride{
				"redeagenda#42": {Model: "issue-42-model"},
			},
		},
	}
	m := newManager(t, cfg)
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 42).Model; got != "issue-42-model" {
		t.Fatalf("issue override: %q", got)
	}
	// 2. Sem issue override, vence o override por repo.
	if got := m.Resolve(domain.RepoRedeAgenda, 7).Model; got != "repo-web-model" {
		t.Fatalf("repo override: %q", got)
	}
	// 3. Repo sem override cai no default do arquivo.
	if got := m.Resolve("other-repo", 0).Model; got != "file-default" {
...
func TestResolveEnvPrecedence(t *testing.T) {
	...
	if got := m.Resolve("other-repo", 0).Model; got != "env-global" {
		t.Fatalf("env global: %q", got)
	}
	if got := m.Resolve(domain.RepoRedeAgenda, 0).Model; got != "repo-web-model" {
	...
	t.Setenv(EnvModelPrefix+"REDEAGENDA", "env-redeagenda")
	if got := m.Resolve(domain.RepoRedeAgenda, 0).Model; got != "env-redeagenda" {
...
func TestResolveIssueOverrideBeatsRepoEnv(t *testing.T) {
	...
	cfg := &Config{
		Model: ModelSection{
			Issues: map[string]ModelOverride{"redeagenda#5": {Model: "issue-5"}},
		},
	}
	m := newManager(t, cfg)
	t.Setenv(EnvModelPrefix+"REDEAGENDA", "env-redeagenda")
	if got := m.Resolve(domain.RepoRedeAgenda, 5).Model; got != "issue-5" {
...
func TestResolveWindow(t *testing.T) {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).ContextWindow; got != 1000000 {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).ContextWindow; got != 200000 {
...
func TestResolveThresholdEnv(t *testing.T) {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).ContextThreshold; got != 0.65 {
		t.Fatalf("threshold arquivo: %v", got)
	}
	// Env válido sobrescreve.
	t.Setenv(EnvContextThreshold, "0.8")
	if got := m.Resolve(domain.RepoRedeAgenda, 0).ContextThreshold; got != 0.8 {
...
func TestLoadAndReload(t *testing.T) {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).Model; got != "first" {
	...
	if got := m.Resolve(domain.RepoRedeAgenda, 0).Model; got != "second" {
