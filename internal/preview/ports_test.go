...
func TestPortManagerRedeAgenda(t *testing.T) {
	cfg := config.PreviewConfig{PortRangeStart: 9000, PortRangeEnd: 9003}
	pm := NewPortManager(cfg)

	// redeagenda precisa de par consecutivo (flutter frontend + go backend).
	p1, e1, err := pm.Alloc(domain.RepoRedeAgenda)
	if err != nil || p1 != 9000 || e1 != 9001 {
		t.Fatalf("alloc 1: port=%d extra=%d err=%v", p1, e1, err)
	}
	p2, e2, err := pm.Alloc(domain.RepoRedeAgenda)
	if err != nil || p2 != 9002 || e2 != 9003 {
		t.Fatalf("alloc 2: port=%d extra=%d err=%v", p2, e2, err)
	}

	// Range esgotado — deve falhar.
	if _, _, err := pm.Alloc(domain.RepoRedeAgenda); err == nil {
		t.Fatal("quero erro quando não há par livre")
	}

	// Liberar o primeiro par e realocar.
	pm.Release(p1, e1)
	p3, e3, err := pm.Alloc(domain.RepoRedeAgenda)
	if err != nil || p3 != 9000 || e3 != 9001 {
		t.Fatalf("alloc após release: port=%d extra=%d err=%v", p3, e3, err)
	}
}
...
func TestPortManagerSinglePort(t *testing.T) {
	...
	// Repos genéricos (sem par obrigatório) alocam 1 porta cada.
	p1, e1, err := pm.Alloc("other-repo")
	if err != nil || p1 != 9000 || e1 != 0 {
		t.Fatalf("alloc 1: port=%d extra=%d err=%v", p1, e1, err)
	}
	p2, _, err := pm.Alloc("other-repo")
	if err != nil || p2 != 9001 {
		t.Fatalf("alloc 2: port=%d err=%v", p2, err)
	}
	p3, _, err := pm.Alloc("other-repo")
	if err != nil || p3 != 9002 {
		t.Fatalf("alloc 3: port=%d err=%v", p3, err)
	}

	// Range esgotado.
	if _, _, err := pm.Alloc("other-repo"); err == nil {
		t.Fatal("quero erro quando range esgotado")
	}

	// Liberar e realocar.
	pm.Release(p2, 0)
	p4, _, err := pm.Alloc("other-repo")
	if err != nil || p4 != p2 {
...
func TestPortManagerMixedAlloc(t *testing.T) {
	...
	// Aloca repo genérico na 9000.
	p, _, _ := pm.Alloc("other-repo")
	if p != 9000 {
		t.Fatalf("esperava 9000, veio %d", p)
	}

	// redeagenda deve pular 9000 (em uso) e pegar 9001+9002.
	wp, we, err := pm.Alloc(domain.RepoRedeAgenda)
	if err != nil || wp != 9001 || we != 9002 {
		t.Fatalf("alloc redeagenda com buraco: port=%d extra=%d err=%v", wp, we, err)
	}
}
